...
Caddy Route Manager atomic skill. Manages reverse-proxy configurations, ingress routing, and Caddyfile reloads using caddy-mcp or direct Caddyfile manipulation.
...
- `systems-manager-mcp` — for direct reading and editing of the global `[REDACTED_POSIX_USER_PATH] configuration.

## Steps

### Step 1: Parse and Locate Target Caddyfile
Verify the location of the active Caddy configuration:
- Locate the primary Caddyfile at `[REDACTED_POSIX_USER_PATH]
...
- Run validation checks on Caddy syntax: `caddy validate --config [REDACTED_POSIX_USER_PATH]
