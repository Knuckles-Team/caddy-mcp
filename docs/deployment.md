...
`caddy-mcp` supports local stdio, a loopback-only development listener, a
...
```bash
caddy-mcp --transport streamable-http --host 127.0.0.1 --port 8000
```
...
docker run -i --rm \
  ...
  -e TRANSPORT=stdio \
  registry.example.invalid/caddy-mcp@sha256:<digest> caddy-mcp
```
...
{
  "mcpServers": {
    "caddy": {"url": "https://service.example.invalid/mcp"}
  }
}
```
...
curl -s "http://technitium.example.invalid:5380/api/zones/records/add" \
  ...
  --data-urlencode "domain=caddy-mcp.example.invalid" \
...
…or add an **A record** `caddy-mcp.example.invalid → <caddy-host-ip>` in the Technitium web
...
For a remote HTTP server, point the client at `http://caddy-mcp.example.invalid/mcp` instead.
